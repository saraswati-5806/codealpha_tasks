...
function App() {
  ...
  const loadPackets = async () => {
    ...
    try {
      ...
      setStatus(
        "Flask API not reachable. Showing demo data. Start python sniffer/api.py."
      );
      setLastUpdated(new Date().toLocaleTimeString());
    }
  };
    ...
    <main className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-slate-300">
              Live packet monitoring dashboard connected with Flask API, Scapy
              sniffer, CSV logging, charts, filters, and threat alerts.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <button
              onClick={loadPackets}
              className="flex items-center gap-2 rounded-xl border border-cyan-400/40 bg-cyan-400/10 px-4 py-3 text-cyan-200 hover:bg-cyan-400/20"
            >
              <RefreshCw size={18} />
              Refresh Data
            </button>
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-emerald-400/40 bg-emerald-400/10 px-4 py-3 text-emerald-200 hover:bg-emerald-400/20 disabled:cursor-not-allowed disabled:opacity-40"
            >
              <Download size={18} />
              Download CSV
            </button>
      ...
      <section className="mx-auto max-w-7xl px-6 py-8">
        <div className="mb-6 grid gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-cyan-400/20 bg-slate-900 p-4 text-sm text-cyan-200">
            Status: {status}
            <br />
            Last updated: {lastUpdated}
          </div>

          <div className="rounded-xl border border-cyan-400/20 bg-slate-900 p-4 text-sm">
        ...
        <div className="mt-8 rounded-2xl border border-red-400/20 bg-slate-900 p-6">
          ...
          <div className="grid gap-3">
            {filteredPackets.filter((p) => p.risk_flag !== "NORMAL").length === 0 ? (
